...
Néanmoins l'apprentissage peut être assez déroutant au départ si l'on ne comprends pas la philosophie sur laquelle est basée ce Framework. Une fois cette étape achevé, il devient très simple de répondre à des problématiques d’enchaînement d’événements complexes.
...
Ils peuvent avoir une durée de vie déterminé (ex : une requête Http) ou non (ex le signal de perte de réseau). De la même manière ces flux peuvent ainsi emmètre un ou plusieurs événements à des intervalles différents.

En Rx un *Observable* sert à matérialiser ces flux, ils peuvent être écoutés, avoir leurs propre cycle de vie, être composés de différentes flux... etc.

### Les Subcribers

Est la notion la plus simple a appréhender. Il s'abonne a un *Observable* et il reçoit ses d'émissions, ses signaux d'erreurs ou fin de traitement.

### Les Operators

Les *Operators* servent à traiter et organiser les flux. Ils permettent de filtrer, combiner, transformer, parcourir, composer des flux etc...

De base une foultitude d'opérateurs sont fournis. Bien qu'on puisse en créer de nouveaux, en pratique les opérateur par défaut sont généralement  largement suffisants pour répondre à la majorité des situations. 
...
Il existe plusieurs méthodes génériques pour créer une *Observable* soit a partir de valeurs fixes, d'une collection, d'un `Future<?>` ou `Callback<?>` ou opérateurs. 
Ils est aussi relativement facile de créer des *Observable* particuliers si l'on respecte son cycle de vie.

Comme vu précédemment par défaut un *Observable* s'execute sur le même Thread que celui qui l'a instancié. En spécifiant `Schedulers.newThread()` on force l’exécution du traitement sur un nouveau Thread. 
Rx est fournit avec plusieurs schedulers prédéfinis tel que `io`, `computation`, etc.. On a aussi la possibilité d'en créer afin d'optimiser la gestion des instances de Thread.
...
- `onError`: lorsqu'une erreur c'est produite lors du traitement.
...
Dans le cas présent, l'*Observable* est exécuté immédiatement avec une durée de vie fixé à un élément. 
...
```
Applique pour chaque éléments une transformation  (il est également possible de modifier le type de retour):
...
Ces opérateurs sont bien évidement combinables entre eux.
